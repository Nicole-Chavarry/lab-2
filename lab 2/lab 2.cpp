...
void mean(std::vector<int> vctCount);
...
int main()
{
	...
	std::cout << "Would you like to add or remove a student? Add - 1, remove - 0. Otherwise, enter any other character." << std::endl;
	int upply;
	std::cin >> upply;
	...
	if (upply == 1)
	{
		...
		std::cout << "Enter the student's number to delete" << std::endl;
		int numberStud;
		std::cin >> numberStud;
...
void firstWay(std::vector<int>& vctCount, std::vector<std::string>& vctName)
{
	vctName = { "Иван", "Паулина", "Кирил", "Миша", "Диана", "Паул", "Карина", "Алехандра", "Артур", "Викторя" };
	vctCount = { 75, 68, 90, 86, 75, 59, 78, 70, 96, 51 };
}
...
void thirdWay(std::vector<int>& vctCount, std::vector<std::string>& vctName)
{
	...
	int ball;
	std::string name;
	std::cout << "Enter the student's name first, then his scores." << std::endl;
	for (int i = 0; i < count; i++)
	{
		std::cin >> name >> ball;
...
void mean(std::vector<int> vctCount)
{
	...
	std::cout << std::endl << "Mean: " << mid / (vctCount.size()) << std::endl;
}
...
void median(std::vector<int> vctCount)
{
	...
	if (!(vctCount.size() % 2))
	{
		std::cout << "Мedian: " << vctCount[midNum] << std::endl;
	}
	else
	{
		std::cout << "Меdian: " << (vctCount[midNum] + vctCount[midNum + 1]) / 2 << std::endl;
	}
}
...
void mode(std::vector<int> vctCount, std::vector<std::string> vctName)
{
	...
	std::cout << "The value of mode: " << mode << ". List of names whose rating corresponds to mode: \n";
